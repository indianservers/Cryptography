...
export default function ISO7816PaddingPage() {
  const [input, setInput] = useState("card payload");
  ...
  const result = useMemo(() => {
    ...
    const padding = [0x80, ...Array(Math.max(0, padLength - 1)).fill(0)];
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="ISO/IEC 7816-4 Padding" category="Padding Schemes" status="Educational">Append a single 0x80 marker byte, then zero-fill the rest of the block.</PageHeader>
      <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
        ...
        <Card title="Padding result">
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <ValueRow label="Marker byte" value="0x80" />
          </div>
      ...
      <Card title="Block visualization">
        <div className="space-y-4">{result.blocks.map((block, blockIndex) => <div key={blockIndex}><div className="mb-2 text-sm font-semibold">Block {blockIndex + 1}</div><div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${Math.min(blockSize, 16)}, minmax(0, 1fr))` }}>{block.map((byte, index) => {
          ...
          const isMarker = absolute === result.data.length;
          return <div key={index} className={`rounded border p-2 text-center font-mono text-xs ${isMarker ? "border-cyan-300 bg-cyan-100 text-cyan-950" : isPadding ? "border-amber-300 bg-amber-100 text-amber-950" : "border-slate-200 bg-slate-50"}`}>{hexByte(byte)}</div>;
        })}</div></div>)}</div>
      </Card>
      <Card title="Warnings and export">
        <WarningBadge>Unpadding scans backward until it finds the 0x80 marker. Any bytes after the marker must be zero.</WarningBadge>
        <div className="mt-4"><ExportReportButton title="ISO 7816 padding" data={result} /></div>
      </Card>
    </div>
  );
}

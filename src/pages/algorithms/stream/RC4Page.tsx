        ...
        <Card title="KSA trace, first 16 swaps">
          <div className="overflow-auto rounded-md border border-slate-200">
            <table className="w-full text-sm">
              ...
              <tbody>{result.ksaTrace.map((row) => <tr key={row.step} className="border-t border-slate-100"><td className="p-2 font-mono">{row.i}</td><td className="p-2 font-mono">{row.j}</td><td className="p-2 font-mono">{row.si}</td><td className="p-2 font-mono">{row.sj}</td></tr>)}</tbody>
            </table>
          </div>
        </Card>
        <Card title="PRGA trace">
          <div className="overflow-auto rounded-md border border-slate-200">
            <table className="w-full text-sm">
              ...
              <tbody>{result.prgaTrace.map((row) => <tr key={row.step} className="border-t border-slate-100"><td className="p-2 font-mono">{row.step}</td><td className="p-2 font-mono">{row.i}</td><td className="p-2 font-mono">{row.j}</td><td className="p-2 font-mono">{row.keystream.toString(16).padStart(2, "0")}</td></tr>)}</tbody>
            </table>
          </div>
        </Card>
      </div>

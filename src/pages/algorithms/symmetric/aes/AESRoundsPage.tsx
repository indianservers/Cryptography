...
export default function AESRoundsPage() {
  ...
  const trace = useMemo(() => buildAesSteps(plain, key, bits), [bits, key, plain]);
  ...
  const setPreset = (next: AesBits) => {
    setBits(next);
    setKey(next === 128 ? "000102030405060708090a0b0c0d0e0f" : next === 192 ? "000102030405060708090a0b0c0d0e0f1011121314151617" : "000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f");
    setStepIndex(0);
  };
    ...
    <div className="space-y-6">
      ...
      <InputPanel title="Round trace inputs">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <label className="label">Key hex ({keyBytes} bytes)<input className="field mt-1 font-mono" value={key} onChange={(event) => { setKey(event.target.value); setStepIndex(0); }} /></label>
        </div>
      </InputPanel>

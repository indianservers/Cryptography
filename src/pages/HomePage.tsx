...
export default function HomePage() {
  ...
  const educationalCount = navigationItems.length - realCount;
  const completionPercent = Math.round((realCount / navigationItems.length) * 100);
      ...
      <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        ...
        <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
          <div className="flex flex-wrap items-end justify-between gap-2">
            <div><div className="text-2xl font-bold text-slate-900">{realCount} of {navigationItems.length}</div><div className="text-sm text-slate-600">pages are currently Web Crypto-backed; other pages are educational or pending verification</div></div>
          ...
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs font-semibold text-slate-600">
            <span>{educationalCount} educational/pending pages</span>
            <span>{supportCounts["Web Crypto"] ?? 0} Web Crypto</span>
            <span>{supportCounts["Custom TypeScript"] ?? 0} custom TypeScript</span>
